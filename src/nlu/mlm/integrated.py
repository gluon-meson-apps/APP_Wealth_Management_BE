...
class IntegratedNLU(Nlu):
    ...
    async def extract_intents_and_entities(self, conversation: ConversationContext) -> IntentWithEntity:
        ...
        use_latest_history = start_new_topic and conversation.current_intent.ignore_previous_slots

        logger.info("extracting utterance's slots")
        current_entities = await self.entity_extractor.extract_entity(conversation, use_latest_history)
        # Retain entities
