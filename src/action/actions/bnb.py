...
class BankRelatedAction(Action):
    ...
    def __init__(
        self, action_name, possible_slots, intent, output_adapter: OutputAdapter
    ):
    ...
    def run(self, context) -> ActionResponse:
        ...
        target_slot_value, target_slot_name = self.get_target_slot_name_and_value(
            target_slots
        )
        slot = self.output_adapter.prepare_slot(
            self.action_name, target_slot_value, target_slot_name
        )
        answer = self.output_adapter.prepare_answer(
            slot,
        ...
        return ActionResponse(
            code=200, message="success", answer=answer, jump_out_flag=False
        )

    def get_target_slots(self):
        target_slots = [
            x
            for x in self.possible_slots
            if x.name in ActionToValidSlotTypesDict[self.action_name]
        ]
    ...
    def get_target_slot_name_and_value(self, target_slots: []):
        target_slot_name = self.output_adapter.get_slot_name(
            self.action_name, target_slots
        )
        ...
        target_slot_value = self.output_adapter.normalize_slot_value(
            slot_value, target_slot_name, self.action_name
        )
...
class JumpOut(Action):
    ...
    def run(self, context) -> ActionResponse:
        ...
        return JumpOutResponse(
            code=200, message="success", answer={}, jump_out_flag=True
        )
