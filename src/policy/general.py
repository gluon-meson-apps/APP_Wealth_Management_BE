...
class EndDialoguePolicy(Policy):
    ...
    def handle(
        ...
        if context.current_intent is not None:
            if (
                context.inquiry_times >= MAX_FOLLOW_UP_TIMES
                and context.current_intent.name not in IN_SCOPE_INTENTS
            ):
                return True, EndDialogueAction()
        return False, None
...
class JumpOutPolicy(Policy):
    ...
    def handle(
        ...
        if IE.intent.name == "chitchat":
            ...
            if len(context.intent_queue) < 2 or not any(
                intent.business for intent in context.intent_queue
            ):
                return True, JumpOut()
            # 尝试向其询问意图
            else:
...
class IntentFillingPolicy(Policy):
    ...
    def handle(
        ...
        if IE.intent is None:
            ...
            return True, IntentFillingAction(
                prompt_manager=self.prompt_manager, form_store=self.form_store
            )

        # 有非辅助外的意图但是置信度低
        if (
            IE.intent.confidence < INTENT_SIG_TRH
            and IE.intent.name in IN_SCOPE_INTENTS
            and context.inquiry_times < MAX_FOLLOW_UP_TIMES
        ):
            context.set_state("intent_confirm")
            return True, IntentConfirmAction(
                IE.intent, prompt_manager=self.prompt_manager
            )
        return False, None
...
class SlotFillingPolicy(Policy):
    ...
    def handle(
        ...
        if form := self.form_store.get_form_from_intent(IE.intent):
            ...
            missed_slots = list(
                filter(lambda slot: slot.optional is not True, missed_slots)
            )
            ...
            if missed_slots and context.inquiry_times < MAX_FOLLOW_UP_TIMES:
                ...
                return True, SlotFillingAction(
                    to_filled_slot, IE.intent, prompt_manager=self.prompt_manager
                )
            ...
            for slot in possible_slots:
                if slot in form.slots and slot.confidence < SLOT_SIG_TRH:
                    ...
                    return True, SlotConfirmAction(
                        IE.intent, slot, prompt_manager=self.prompt_manager
                    )
            ...
            if form.slot_required and context.inquiry_times < MAX_FOLLOW_UP_TIMES:
                ...
                if optional_slots and len(possible_slots) == 0:
                    ...
                    context.set_state(f"slot_filling:{to_filled_slot.name}")
                    return True, SlotFillingAction(
                        optional_slots, IE.intent, prompt_manager=self.prompt_manager
                    )
...
class AssistantPolicy(Policy):
    ...
    def handle(
        ...
        if (
            IE.intent.name in IN_SCOPE_INTENTS
            and IE.intent.confidence > INTENT_SIG_TRH
            and context.inquiry_times >= MAX_FOLLOW_UP_TIMES
        ):
            return True, BankRelatedAction(
                inten_form.action, potential_slots, IE.intent, BaseOutputAdapter()
            )
